...
class Datos_Baja(models.Model):  
    ...
    diacnostico = models.TextField('Diacnostico',max_length=255, blank=True, null=True)
    class Meta:
    ...
    def save(self, *args, **kwargs):
        ...
        self.diacnostico = (self.diacnostico).upper()
        self.tecico_diacnosticador = (self.tecico_diacnosticador).upper()
        return super(Datos_Baja, self).save(*args, **kwargs)


    def __str__(self):
        return 'Folio %s %s %s %s %s %s %s %s %s '%(self.Folio,self.fecha_de_baja,self.ticket_baja,self.usuario,self.cargo,self.telefono,self.extencion_del_area,self.asignacion_de_equipos,self.diacnostico)
